...
RSpec.describe LineItems::ProfileTriggers::ToBeFixer do
  ...
  let!(:to_be_fixed_category) { create :profile_category, profile: profile, status: :to_be_fixed, category: shirt_category }
  ...
  let!(:item1) { create :line_item, state: :to_be_reviewed_profile, order: order, product: shirt_product, manufacturer: :new_manufacturer }
  let!(:item2) { create :line_item, state: :no_measurement_profile, order: order, product: shirt_product, manufacturer: :new_manufacturer }
  let!(:item3) { create :line_item, state: :waiting_for_confirmation, order: order, product: pants_product }

  subject { described_class.to_be_fixed(customer: profile.customer, user_id: user.id) }
  ...
  it 'changes the item states properly' do
    ...
    expect(item1.state).to eq 'to_be_fixed_profile'
    expect(item2.state).to eq 'no_measurement_profile'
