...
RSpec.describe LineItems::ProfileTriggers::ToBeChecker do
  ...
  let!(:to_be_reviewed_category) { create :profile_category, profile: profile, status: :to_be_checked, category: shirt_category }
  ...
  let!(:item1) { create :line_item, state: :no_measurement_profile, order: order, product: shirt_product, manufacturer: :new_manufacturer }
  ...
  subject { described_class.to_be_checked(customer: profile.customer, user_id: user.id) }
  ...
  it 'changes the item states properly' do
    ...
    expect(item1.state).to eq 'to_be_checked_profile'
