...
RSpec.describe LineItems::ProfileTriggers::Submitter do
  ...
  let!(:submitted_category) { create :profile_category, profile: profile, status: :submitted, category: shirt_category }
  ...
  subject { described_class.submit(customer: profile.customer, user_id: user.id) }
  ...
  it 'changes the item states properly' do
    ...
    expect(item1.state).to eq 'submitted_profile'
